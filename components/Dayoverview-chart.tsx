...
export function DayOverview({ dateRange }: Props) {
  ...
  const fetchData = async () => {
    ...
    try {
      const url = `/api/sales?name=get_daily_sales&start_date=${start}&end_date=${end}`
  ...
  //서버/클라이언트 style 불일치 방지를 위한 렌더링 조건
  if (!mounted) return null // or <div style={{ height: 400 }} />
